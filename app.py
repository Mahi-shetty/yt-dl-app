...
RAILWAY FIX: Added multiple layers to bypass YouTube's bot detection on cloud IPs:
  1. extractor-args to use web client instead of android/tv (avoids sign-in wall)
  2. Custom user-agent mimicking a real browser
  3. sleep-interval to avoid rate limiting
  4. Mark as not a playlist cleanly
  5. Fallback format strategies for restricted videos
"""
...
# ── Bypass flags for Railway cloud environment ────────────────────────────────
# YouTube blocks cloud IPs and headless requests.
# These args make yt-dlp appear as a real browser session.
BYPASS_ARGS = [
    # Use the web client — avoids the "sign in to confirm age" wall that
    # the default android/tv clients hit on cloud IPs
    "--extractor-args", "youtube:player_client=web",
    # Realistic browser user-agent
    "--user-agent",
    "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 "
    "(KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36",
    # Randomise sleep to avoid rate-limit triggers
    "--sleep-interval", "1",
    "--max-sleep-interval", "3",
    # Don't cache anything (Railway has ephemeral FS)
    "--no-cache-dir",
]
...
def run_ytdlp_no_bypass(args: list) -> tuple:
    """Run without bypass args — used for fallback attempts."""
    if isinstance(YTDLP, list):
        cmd = YTDLP + args
    else:
        cmd = [YTDLP] + args
...
def api_info():
    ...
    # Primary attempt: web client with bypass flags
    stdout, stderr, code = run_ytdlp(["--dump-json", "--no-playlist", url])

    # Fallback: try tv_embedded client if web client still fails
    if code != 0:
        stdout, stderr, code = run_ytdlp_no_bypass([
    ...
    if code != 0:
        ...
        if "sign in" in err or "age" in err:
            return jsonify({
                "error": (
                    "YouTube is blocking this request from the server's IP. "
                    "This is a known issue with cloud deployments — YouTube "
                    "requires sign-in for some videos when accessed from "
                    "data-center IPs. Try a different video, or set up "
                    "cookie-based auth (see README)."
                )
            }), 403
        ...
        if "429" in stderr or "too many" in err:
            return jsonify({"error": "Rate limited by YouTube. Please wait a minute and try again."}), 429
