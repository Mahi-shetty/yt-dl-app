"""
YouTube Downloader — Flask Backend (Railway + Cookie Auth)
"""
...
# Set the COOKIE_FILE environment variable in Railway dashboard to the path
# of your cookies.txt file (e.g. /app/cookies.txt).
# Upload cookies.txt to your repo and Railway will include it in /app/.
COOKIE_FILE = os.environ.get("COOKIE_FILE", "cookies.txt")

def get_cookie_args():
    """Return --cookies arg if the cookie file exists, else empty list."""
    path = Path(COOKIE_FILE)
    if path.exists() and path.stat().st_size > 0:
        return ["--cookies", str(path)]
    return []

# ── Base yt-dlp args (always applied) ────────────────────────────────────────
BASE_ARGS = [
    "--user-agent",
...
def run_ytdlp_with_strategies(args):
    """
    Try each client strategy in order, with cookies if available.
    Returns (stdout, stderr, code) from the first successful attempt,
    or the last failure if all strategies fail.
    """
    base = YTDLP if isinstance(YTDLP, list) else [YTDLP]
    cookie_args = get_cookie_args()

    last_stdout, last_stderr, last_code = "", "", 1

    for strategy in CLIENT_STRATEGIES:
        cmd = base + BASE_ARGS + cookie_args + strategy + args
        ...
        if last_code == 0:
            return last_stdout, last_stderr, 0

        # If it's a hard block (sign-in / age gate), try next strategy
        err = last_stderr.lower()
        if any(kw in err for kw in ("sign in", "age", "login", "private", "unavailable")):
            continue

        # For other errors (network, format, etc.) don't waste time retrying
        break
...
def api_info():
    ...
    stdout, stderr, code = run_ytdlp_with_strategies(["--dump-json", url])

    if code != 0:
        err = stderr.lower()
        if "sign in" in err or "age" in err or "login" in err:
            has_cookies = Path(COOKIE_FILE).exists()
            if has_cookies:
                msg = "YouTube is still blocking this IP even with cookies. The cookies may be expired — re-export them from your browser."
            else:
                msg = (
                    "YouTube is blocking requests from this server's IP. "
                    "Fix: export your YouTube cookies (see README) and add "
                    "cookies.txt to your repo."
                )
    ...
    if audio_streams:
        ...
        for tier in [
            ...
            audio_fmts.append({
                ...
                "ext": "MP3", "bitrate": tier["bitrate"],
                "label": tier["label"], "quality": tier["quality"],
...
def api_download():
    ...
    stdout, stderr, code = run_ytdlp_with_strategies(args)
    if code != 0:
...
if __name__ == "__main__":
    cookie_status = "found ✓" if Path(COOKIE_FILE).exists() else "NOT FOUND ✗"
    print(f"yt-dlp : {YTDLP}")
    print(f"cookies: {COOKIE_FILE} ({cookie_status})")
