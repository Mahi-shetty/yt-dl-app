...
def get_po_token_args():
    if PO_TOKEN and VISITOR_DATA:
        ...
        return ["--extractor-args", f"youtube:visitor_data={VISITOR_DATA}"]
    return []
...
def get_strategies():
    ...
    if po:
        strategies.append(cookie + po + ["--extractor-args", "youtube:player_client=web"])
...
# ── Helpers ───────────────────────────────────────────────────────────────────
# More permissive YouTube URL validation - accepts any youtube.com or youtu.be link
...
def is_valid_youtube_url(url):
    """Accepts any YouTube URL format including tracking params."""
...
def api_info():
    ...
    if not is_valid_youtube_url(url):
        return jsonify({"error": "Invalid YouTube URL. Please paste a valid YouTube link."}), 400
...
if __name__ == "__main__":
    ...
    visitor_status = "✓ set" if VISITOR_DATA else "✗ not set"
    print(f"yt-dlp       : {YTDLP}")
    print(f"cookies.txt  : {cookie_status}")
    print(f"VISITOR_DATA : {visitor_status}")
