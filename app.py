...
def get_strategies():
    ...
    if po:
        strategies.append(
            cookie + po + ["--extractor-args", "youtube:player_client=web"]
        )

    strategies.append(cookie + ["--extractor-args", "youtube:player_client=web"])
    strategies.append(
        cookie + ["--extractor-args", "youtube:player_client=tv_embedded"]
    )
...
def run_ytdlp(args):
    ...
    for strategy in get_strategies():
        ...
        is_block = any(
            k in err for k in ("sign in", "age", "login", "inappropriate", "bot")
        )
        if not is_block:
            break
...
def api_info():
    ...
    if code != 0:
        ...
        if "429" in stderr or "too many" in err:
            return (
                jsonify(
                    {"error": "Rate limited by YouTube. Wait a minute and try again."}
                ),
                429,
            )
    ...
    best_audio_abr = (
        (best_audio.get("abr") or best_audio.get("tbr") or 128) if best_audio else 128
    )
    ...
    for f in all_fmts:
        # Only video-only streams (DASH video)
            ...
            video_fmts.append(
                {
                    ...
                    "resolution": build_format_label(height),
                    "height": height,
                    "ext": "MP4",
                    "filesize": (
                        human_size(vid_bytes + aud_bytes)
                        if (vid_bytes + aud_bytes) > 0
                        else "~"
                    ),
                    "fps": f.get("fps"),
...
def api_download():
    ...
    # Always merge selected video with best available audio
        if format_id:
...
def api_subtitle():
    ...
    run_ytdlp(
        [
            ...
            "srt",
            "--output",
            os.path.join(tmp_dir, "sub.%(ext)s"),
            "--quiet",
            url,
        ]
    )
    ...
    srt_file = next(
        (f for f in Path(tmp_dir).iterdir() if f.suffix.lower() == ".srt"), None
    )
    if not srt_file:
    ...
    return Response(
        ...
        mimetype="text/plain",
        headers={
            "Content-Disposition": f'attachment; filename="subtitles_{sub_lang}.srt"'
        },
    )
