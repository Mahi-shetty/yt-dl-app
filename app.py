"""
YouTube Downloader — Flask Backend (Railway + Cookie + PO Token)
"""
...
PO_TOKEN    = os.environ.get("PO_TOKEN", "")      # optional but helps a lot
VISITOR_DATA = os.environ.get("VISITOR_DATA", "") # paired with PO_TOKEN
...
def get_po_token_args():
    """
    PO Token tells YouTube this is a real browser session.
    Get it from: https://github.com/iv-org/youtube-trusted-session-generator
    Set PO_TOKEN and VISITOR_DATA as Railway environment variables.
    """
...
# Client strategies tried in order (best → fallback)
# Each is a list of extra args
def get_strategies():
    po = get_po_token_args()
    cookie = get_cookie_args()
    strategies = []

    # 1. web + PO token + cookies (best)
    if po:
        strategies.append(
            cookie + ["--extractor-args", f"youtube:player_client=web;{po[1].split(';')[0].split('+')[1] if '+' in po[1] else ''}"] + po
        )

    # 2. web + cookies
    strategies.append(cookie + ["--extractor-args", "youtube:player_client=web"])

    # 3. tv_embedded + cookies (bypasses age gate)
    strategies.append(cookie + ["--extractor-args", "youtube:player_client=tv_embedded"])

    # 4. mweb + cookies
    strategies.append(cookie + ["--extractor-args", "youtube:player_client=mweb"])

    # 5. web_creator + cookies
    strategies.append(cookie + ["--extractor-args", "youtube:player_client=web_creator"])

    # 6. android + cookies
    strategies.append(cookie + ["--extractor-args", "youtube:player_client=android"])

    # 7. No client override, just cookies
    strategies.append(cookie)

    # 8. Bare (no cookies, no client) — last resort
    strategies.append([])
...
def run_ytdlp(args):
    """Try all strategies, return first success or last failure."""
    base = YTDLP if isinstance(YTDLP, list) else [YTDLP]
    last = ("", "All strategies failed.", 1)

    for strategy in get_strategies():
        cmd = base + BASE_ARGS + strategy + args
        ...
        last = (result.stdout, result.stderr, result.returncode)

        # Only retry on auth/block errors, bail early on other errors
        err = result.stderr.lower()
        is_block = any(k in err for k in ("sign in", "age", "login", "inappropriate", "bot"))
        if not is_block:
            break
...
def api_info():
    ...
    stdout, stderr, code = run_ytdlp(["--dump-json", url])
    ...
    if code != 0:
        ...
        if any(k in err for k in ("sign in", "age", "login", "bot", "inappropriate")):
            return jsonify({"error": (
                ...
                "Fix: add PO_TOKEN + VISITOR_DATA to Railway environment variables. "
                "Get them free at: https://github.com/iv-org/youtube-trusted-session-generator"
            )}), 403
        ...
        if "429" in stderr or "too many" in err:
            return jsonify({"error": "Rate limited by YouTube. Wait a minute and try again."}), 429
    ...
    if audio_streams:
        ...
        for tier in [
            ...
            audio_fmts.append({
                "format_id": best_audio_id, "ext": "MP3",
                "bitrate": tier["bitrate"], "label": tier["label"],
                "quality": tier["quality"], "src_abr": f"{src_abr}kbps source",
            })
        m4a_abr = int(best_audio.get("abr") or best_audio.get("tbr") or 128)
        audio_fmts.append({
            "format_id": best_audio_id, "ext": "M4A",
            "bitrate": str(m4a_abr), "label": f"{m4a_abr} kbps",
            "quality": "Original AAC (no re-encode)", "src_abr": "native",
        })
...
def api_download():
    ...
    if media_type == "audio":
        if ext == "M4A":
            args = [
                "--format",
                f"{format_id}/bestaudio[ext=m4a]/bestaudio/best" if format_id else "bestaudio[ext=m4a]/bestaudio/best",
                "--output", out_tmpl, url,
            ]
        else:
            args = [
...
if __name__ == "__main__":
    cookie_status = "✓ found" if Path(COOKIE_FILE).exists() else "✗ missing"
    po_status     = "✓ set" if PO_TOKEN else "✗ not set"
    print(f"yt-dlp      : {YTDLP}")
    print(f"cookies.txt : {cookie_status}")
    print(f"PO_TOKEN    : {po_status}")
